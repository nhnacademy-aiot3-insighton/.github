...
| 영역 | 요구사항 | 담당 |
|---|---|-----|
| 실시간 관제 | 공간별 센서 데이터(CO2, 온도, 습도)를 실시간 차트로 확인 | CORE |
| 실시간 관제 | 대시보드를 위젯 단위로 자유롭게 구성(추가/배치/크기 조정) | CORE |
| 기기 제어 | 액추에이터(에어컨, 공기청정기, 환기팬 등) 대시보드 수동 원격 제어 | CORE |
| 인프라 관리 | 게이트웨이·기기의 등록 현황과 정상/장애 상태 조회 | INFRA |
| 신규 기기 인식 | 미등록 기기의 패킷이 최초 수신되면 자동으로 장비 등록 | **CORE**, ENGINE |
| 규칙 자동화 | 코드 작성 없이 트리거·조건·액션을 조합한 자동화 규칙 생성 | ENGINE |
| 규칙 자동화 | 센서 임계치 기반뿐 아니라 특정 시각(cron) 기반 실행 지원 | ENGINE |
| 규칙 자동화 | 동일 조건 반복 발생 시 알림·제어 과다 반복 방지(빈도 제한) | ENGINE |
| AI 제어 | 공간별 "AI 제안만 받기" / "AI가 직접 제어하기" 모드 선택 | CORE, **AI** |
| AI 제어 | 실내 센서 추이 + 실시간 외부 날씨·미세먼지 결합 제어 제안 | CORE, **AI** , API|
| AI 제어 | "제안 받기" 모드에서 수락 시 즉시 실제 기기 제어로 연결 | CORE |
| 알림 | 임계치 초과 등 이상 상황 대시보드 즉시 알림 표시 | **CORE**, ENGINE |
| 알림 | Telegram/이메일로도 알림 수신 | **CORE**, ENGINE |
| 리포트 | 매시간 센서 통계(평균/최고/최저) 및 기기별 가동 시간 자동 집계 | CORE |
| 리포트 | 주간/월간 에너지 효율·쾌적도 진단 리포트 자동 생성 | CORE |
| 이력/감사 | 모든 기기 제어 이력을 실행 주체(유저/AI/규칙엔진)별로 조회 | CORE |
| 조직 관리 | 초대 토큰 하나로 멤버 셀프 가입 | AUTH |
| 조직 관리 | 역할(소유자/관리자/멤버)별 접근 권한 차등 | AUTH |
| 계정 | 이메일/비밀번호 또는 소셜 로그인(OAuth) 지원 | AUTH |
