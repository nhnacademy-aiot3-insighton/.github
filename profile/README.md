...
| ID | 요구사항 | 상세 설명 / 수용 기준 | 우선순위 | 의존성 |
| --- | --- | --- | --- | --- | --- |
| FR-24 | 회원가입/로그인 | 이메일 가입 | 이메일+비밀번호 가입 (BCrypt 해시, 이메일 소문자 정규화·중복 거부) | Must | `users`, `user_credentials` |
| FR-24 | 회원가입/로그인 | 로그인 | 성공 시 Access·Refresh 발급, 실패 사유는 계정 열거 방지를 위해 통합 응답 | Must | `users`, `user_credentials` |
| FR-24 | 회원가입/로그인 | 로그인 잠금 | 5회 연속 실패 시 5분 잠금 | Must | Redis |
| FR-25 | 소셜 로그인/연동 | OAuth 로그인 | 소셜 계정으로 로그인·가입 (제공자 TBD) | Must | `oauth` |
| FR-25 | 소셜 로그인/연동 | 계정 연동 질의 | 미연결 수단인데 동일 이메일 계정 존재 시 연동 여부 질의 | Must | `oauth` |
| FR-25 | 소셜 로그인/연동 | 연동 확정/거부 | 동의 시 기존 비밀번호로 본인 확인 후 연결, 거부 시 가입 중단·기존 수단 안내 (동일 이메일 복수 계정 금지) | Must | `oauth` |
| FR-26 | 토큰 발급·검증 | 서명·클레임 | RS256 비대칭키 서명, 권한 클레임 포함, Access TBD분 / Refresh 14일 | Must | — |
| FR-26 | 토큰 발급·검증 | 저장 위치 | Access=프론트 메모리, Refresh=httpOnly 쿠키 + Redis | Must | Redis |
| FR-26 | 토큰 발급·검증 | Gateway 검증 | Gateway가 서명·만료·권한 검증 후 `X-User-Id`·`X-User-Role` 전달 (매 요청 저장소 조회 없음) | Must | Gateway |
| FR-27 | 재발급·로그아웃 | 재발급(회전) | Access 만료 시 Refresh 회전 재발급 (Redis 조회는 재발급 시점에만) | Must | Redis |
| FR-27 | 재발급·로그아웃 | 재사용 탐지 | 폐기된 Refresh 재제출 시 해당 사용자 계열 전체 무효화 | Must | Redis |
| FR-27 | 재발급·로그아웃 | 로그아웃 | 클라이언트 토큰 제거 + 서버 Refresh 폐기, Access는 프론트 폐기(블랙리스트) | Must | Redis |
| FR-28 | 계정 관리 | 아이디 찾기 | 이메일 마스킹 표시 | Must | — |
| FR-28 | 계정 관리 | 비밀번호 찾기 | 일회성·만료 링크로 재설정 | Must | — |
| FR-28 | 계정 관리 | 비밀번호 변경 | 현재 비밀번호 확인 후 변경 | Must | — |
| FR-28 | 계정 관리 | 내 정보 조회·수정 | 내 정보 조회 및 수정 | Must | — |
| FR-28 | 계정 관리 | 연결 수단 관리 | 연동 수단 관리 (마지막 수단 해제 불가) | Must | `oauth` |
| FR-28 | 계정 관리 | 탈퇴 | 물리 삭제 대신 상태 전환·이력 보존, 변경·재설정·탈퇴 시 Refresh 폐기 | Must | Redis |
| FR-29 | 접근 제어(RBAC) | 역할 체계 | ADMIN/MEMBER 2단계, 가입 시 기본 MEMBER | Must | 권한 모델 |
| FR-29 | 접근 제어(RBAC) | 인가 판단 | 토큰 권한 클레임으로 인가, 권한 부족 시 403 | Must | 권한 모델 |
| FR-29 | 접근 제어(RBAC) | 관리자 기능 | ADMIN이 구성원 목록 조회·역할 변경·블록 (다음 토큰 재발급 시 반영) | Must | 권한 모델 |
| FR-31 | 이력·보안 | 접근 이력 | 로그인/로그아웃/정보변경 시 IP·User-Agent 기록 (3개월 초과 삭제, 토큰·비밀번호 원문 저장 금지) | Must | 로그 저장소 |
| FR-31 | 이력·보안 | 감사 로그 | 장비 제어는 감사 로그 별도 | Must | 로그 저장소 |
| FR-31 | 이력·보안 | 휴면 전환 | 스케줄러 + Redis 락으로 휴면 전환 | Must | 스케줄러 |
