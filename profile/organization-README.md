...
> ⚠️ 담당 파트 컬럼이 비어 있습니다. 팀장 피드백과 별개로, 팀원별 이름/담당 서비스는 조립 전 채워 넣어야 합니다.
...
<details>
<summary>2. 요구사항 (요약 — 상세는 별도 문서)</summary>
<br/>

> 아래 표는 개요이며, ID·수용 기준·우선순위·의존성까지 포함한 전체 명세는 [`functional-requirements.md`](./functional-requirements.md)를 참고합니다. (팀장 피드백: "기능 요구사항이 더 구체적으로 정의되어야 한다" 반영)

**기능 요구사항**

| 영역 | 요구사항 | 담당 | 상세 |
|---|---|-----|---|
| 실시간 관제 | 공간별 센서 데이터(CO2, 온도, 습도)를 실시간 차트로 확인 | CORE | FR-01 |
| 실시간 관제 | 대시보드를 위젯 단위로 자유롭게 구성(추가/배치/크기 조정) | CORE | FR-02 |
| 기기 제어 | 액추에이터(에어컨, 공기청정기, 환기팬 등) 대시보드 수동 원격 제어 | CORE | FR-03 |
| 인프라 관리 | 게이트웨이·기기의 등록 현황과 정상/장애 상태 조회 | INFRA | FR-04 |
| 신규 기기 인식 | 미등록 기기의 패킷이 최초 수신되면 자동으로 장비 등록 | **CORE**, ENGINE | FR-05 |
| 규칙 자동화 | 코드 작성 없이 트리거·조건·액션을 조합한 자동화 규칙 생성 | ENGINE | FR-06 |
| 규칙 자동화 | 센서 임계치 기반뿐 아니라 특정 시각(cron) 기반 실행 지원 | ENGINE | FR-07, FR-08 |
| 규칙 자동화 | 동일 조건 반복 발생 시 알림·제어 과다 반복 방지(빈도 제한) | ENGINE | FR-10 |
| AI 제어 | 공간별 "AI 제안만 받기" / "AI가 직접 제어하기" 모드 선택 | CORE, **AI** | FR-15 |
| AI 제어 | 실내 센서 추이 + 실시간 외부 날씨·미세먼지 결합 제어 제안 | CORE, **AI**, API | FR-16 |
| AI 제어 | "제안 받기" 모드에서 수락 시 즉시 실제 기기 제어로 연결 | CORE | FR-17 |
| 알림 | 임계치 초과 등 이상 상황 대시보드 즉시 알림 표시 | **CORE**, ENGINE | FR-19 ⚠️ 전송 방식 미확정 |
| 알림 | Telegram/이메일로도 알림 수신 | **CORE**, ENGINE | FR-14, FR-20 |
| 리포트 | 매시간 센서 통계(평균/최고/최저) 및 기기별 가동 시간 자동 집계 | **AI** (README 초안 오탈자 정정 — 4.1절 근거) | FR-21 |
| 리포트 | 주간/월간 에너지 효율·쾌적도 진단 리포트 자동 생성 | **AI** (동일 정정) | FR-22 |
| 이력/감사 | 모든 기기 제어 이력을 실행 주체(유저/AI/규칙엔진)별로 조회 | CORE | FR-23 |
| 조직 관리 | 초대 토큰 하나로 멤버 셀프 가입 | AUTH | FR-24 |
| 조직 관리 | 역할(소유자/관리자/멤버)별 접근 권한 차등 | AUTH | FR-25 |
| 계정 | 이메일/비밀번호 또는 소셜 로그인(OAuth) 지원 | AUTH | FR-26 |

**비기능 요구사항**

| 구분 | 요구사항 | 상세 |
|---|---|---|
| 실시간성 | 수집 스레드는 논블로킹 즉시 반환, 저장/전파는 비동기 분리 | NFR-01 |
| 장애 격리 | 시계열 저장소(InfluxDB) 장애가 실시간 수집 전체를 막지 않음(Fail-Silent) | NFR-02 |
| 멀티테넌시 | 고객사(그룹) 간 데이터 물리적 격리, 서비스별 독립 DB | NFR-03 |
| 확장성 | 새 센서 항목/액추에이터 종류 추가 시에도 스키마 변경 없이 수용(JSONB) | NFR-04 |
| 확장성 | 대형 고객사 트래픽 증가 시 다른 고객사 영향 없이 별도 자원 배정 가능, 멀티 인스턴스 배치 중복 실행 방지(ShedLock) | NFR-05 |
| 비용 효율 | LLM 호출은 정기 배치가 아닌 이상 상황 발생 시에만 트리거 | NFR-06 |
| 성능 | 반복 조회되는 매핑 정보는 인메모리 캐시로 DB 조회 병목 방지 | NFR-07 |
| 보안 | 유출된 초대 토큰은 즉시 재발급(무효화) 가능 | NFR-08 |
| 데이터 무결성 | 동일 물리 DB 내 테이블 간 참조 무결성을 실제 제약조건으로 보장 | NFR-09 |
| 오버 엔지니어링 회피 | 실제 운영 필요성이 확인되기 전까지 불필요한 이력 저장·과도한 트랜잭션 보장 도입 지양 | NFR-10 |

> ⚠️ 미확정 항목(임의로 값을 채우지 않음): 실시간 알람 푸시 방식(WebSocket/SSE/폴링), 실시간성 목표 지연 시간, 게이트웨이 하트비트 임계 시간, OAuth 제공자 목록, 역할별 접근 권한 매트릭스. 자세한 목록은 `functional-requirements.md` 3장 참고.
...
> 팀장 피드백("현재 그려진 것을 아키텍처라 부를 수는 없다")을 반영해, 컴포넌트 박스 다이어그램 한 장이 아니라 **(1) 컨테이너 다이어그램(프로토콜/동기·비동기 명시) + (2) 핵심 흐름 3개의 시퀀스 다이어그램 + (3) 데이터 저장 전략**으로 구성했습니다.

### 3.1 컨테이너 다이어그램

MSA(5개 마이크로서비스) 구조로, 서비스 간 물리 DB는 완전히 분리되고 논리 키 매핑으로 연동됩니다. 화살표에는 프로토콜과 동기(●)/비동기(○) 여부를 표기했습니다.
...
- [`smart-office-iot-architecture-spec.md`](./smart-office-iot-architecture-spec.md) — Core/AI 서비스 엔티티·파이프라인 상세, ERD 갭 보정
- [`rule_engine.md`](./rule_engine.md) — Rule Engine 노드 타입/실행 컨텍스트 상세
- [`erd-table-memos.md`](./erd-table-memos.md) — 테이블별 설계 의도
- [`functional-requirements.md`](./functional-requirements.md) — 기능/비기능 요구사항 상세 명세
- [`wbs.md`](./wbs.md) — 서비스별 작업 분해 구조

<br/>

## 🗂️ WBS (Work Breakdown Structure)

> 팀장 피드백("WBS 작성이 미흡하다")을 반영해 서비스별·단계별 작업 분해를 별도 문서로 분리했습니다. 요약:

| Phase | 목표 |
|---|---|
| Phase 0 | 설계 확정 — ERD 갭 반영 최종 DDL, 서비스 간 API/이벤트 스펙 확정 |
| Phase 1 | 데이터 계층 — 서비스별 Entity/DTO/Repository/API |
| Phase 2 | 핵심 로직 — 실시간 파이프라인/배치/규칙 실행/LLM 연동 |
| Phase 3 | 서비스 간 연동 — Feign 호출, RabbitMQ 발행/구독 |
| Phase 4 | 안정화 — ShedLock 다중 인스턴스 검증, 미확정 항목 확정 |

서비스별(Gateway/Auth/Core/Rule Engine/AI) 세부 작업·의존순서·산출물은 [`wbs.md`](./wbs.md) 전체 참고. **AI/Rule Engine은 기존 이슈·설계 문서 기준으로 작성되어 근거가 명확하지만, Auth/Gateway는 README 요구사항만으로 추정한 것이라 담당자 확인이 필요합니다.**
